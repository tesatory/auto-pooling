function XC = extract_features_ae3x3(X, W, bh, rfSize, CIFAR_DIM, M,P,stride)
  whitening = stride;
  ...
  XC = zeros(size(X,1), numCentroids*9);
  for i=1:size(X,1)
    ...
    q13 = sum(sum(patches(1:halfr, 1+halfc*2:end, :), 1),2);
    q21 = sum(sum(patches(1+halfr:halfr*2, 1:halfc, :), 1),2);
    q22 = sum(sum(patches(1+halfr:halfr*2, 1+halfc:halfc*2, :), 1),2);
    q23 = sum(sum(patches(1+halfr:halfr*2, 1+halfc*2:end, :), 1),2);
    q31 = sum(sum(patches(1+halfr*2:end, 1:halfc, :), 1),2);
    q32 = sum(sum(patches(1+halfr*2:end, 1+halfc:halfc*2, :), 1),2);
    q33 = sum(sum(patches(1+halfr*2:end, 1+halfc*2:end, :), 1),2);
    
    % concatenate into feature vector
    XC(i,:) = [q11(:);q12(:);q13(:);q21(:);q22(:);q23(:);q31(:);q32(:);q33(:)]';
  end
